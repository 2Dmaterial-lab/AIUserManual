...
| 场景 | 建议 | 原因 |
|------|------|------|
| 快速查看光谱、IV、PL、拉曼等二维数据 | 推荐 | 导入、绘图和拟合入口直观 |
| 少量论文图排版 | 推荐 | 图层、坐标轴和模板调整方便 |
| 批量生成同格式图 | 推荐脚本化使用 | 使用 LabTalk 或 `originpro` 才可复现 |
| 大规模数据清洗和统计建模 | 谨慎 | MATLAB/Python 更适合版本化和自动测试 |
| 长期保存分析流程 | 谨慎 | 不要只保存 `.opju`，应同时保存脚本和导出数据 |

### AI 能做 / 不能做

| AI 适合做 | 必须人工确认 |
|-----------|--------------|
| 生成 LabTalk / Python 导入和绘图脚本 | 列含义、单位、异常值处理 |
| 规划图层、坐标轴、拟合流程 | 拟合模型是否有物理意义 |
| 解读 Origin 报错和菜单位置 | 参数初值、约束、置信区间 |
| 把手动操作整理成批处理流程 | 论文图是否可追溯到原始数据 |

### 最小闭环案例

1. 从 `docs/assets/test_data/data1.csv` 导入两列数据。
2. 用 AI 生成一段 `originpro` 或 LabTalk 脚本，画波长-透过率曲线。
3. 导出 `PNG/PDF`，同时保存脚本和处理后的数据。
4. 检查坐标轴单位、线宽、字体和数据范围是否与原始 CSV 一致。
...
- **图表保护与模板管理**：新版图表保护、参考线、切片/筛选等能力更适合维护论文级图模板。
...
1. **先用副本测试**：Origin 脚本可能直接修改工作簿、图层和项目结构，正式数据前先复制 `.opju` 项目。
2. **明确版本和接口**：提问时写明 Origin 版本、希望使用 LabTalk 还是 `originpro` Python。
3. **检查输入输出**：确认文件路径、列号、长名称、单位和缺失值处理方式。
4. **保留可复现记录**：保存导入模板、图模板、脚本文件和导出图参数。
5. **核对拟合结果**：AI 生成的拟合脚本要检查初值、约束、权重、残差图和参数置信区间。
...
- Origin Release Notes：[originlab.com/ReleaseNotes](https://www.originlab.com/ReleaseNotes)
